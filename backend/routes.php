...
// Captura erros não fatais (error handler) - Converte todos os erros em exceções
...
try {
    ...
    // A conexão é fechada implicitamente no final do script ou com exit()
    // $conn->close(); // Esta linha é redundante e inalcançável

} catch (Throwable $e) {
    // Captura qualquer erro ou exceção que não foi tratado
    sendFatalErrorResponse("Exceção capturada: " . $e->getMessage() . " em " . $e->getFile() . " na linha " . $e->getLine());
}

// --- Funções de Manipulação de Rota (Handlers) ---

/**
 * Manipula as requisições de geocodificação, atuando como um proxy para o Nominatim.
 */
function handleGeocode() {
    // Este endpoint foi desativado em favor de uma chamada do lado do cliente 
    // via um proxy CORS para contornar limitações de SSL do servidor.
    sendResponse(410, array('error' => 'Este endpoint de geocodificação está obsoleto.'));
}

/**
 * Manipula o upload do arquivo XML da NF-e.
 */
